...
const Login: React.FC = () => {
  ...
  // const [loginType, setLoginType] = useState<'user' | 'admin'>('user'); // Removed
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Enforce role restrictions based on selected tab
      if (role === 'Patient') {
        // "Patient" tab allows Patients and App Admins (Super Admin)
        if (user.role !== 'Patient' && user.role !== 'Admin' && user.role !== 'Super Admin') {
          throw new Error('This login is for Patients and App Admins only.');
        }
      } else if (role === 'Clinic Admin') {
        // "Clinic Admin" tab allows only Clinic Admins
        if (user.role !== 'Clinic Admin') {
          throw new Error('This login is for Clinic Admins only.');
        }
      }
      ...
      // Convert network/fetch errors to user-friendly message
        ...
        <div className="frosted-glass p-6 md:p-8 rounded-[2.5rem] shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed text-white py-4 rounded-2xl font-bold shadow-lg shadow-blue-50 transition-all flex items-center justify-center"
            >
              {loading ? (
                <>
